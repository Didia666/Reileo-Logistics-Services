...
switch ($method) {
    ...
    case 'GET':
        try {
            if ($id > 0) {
                $baseSql = listSql($tblVehicles, $tblModels, $tblManufacturers, $tblTypes, $tblCategoryTypes, $tblVendors, $tblVehicleStatuses, $tblLocation, $tblOrigins, $tblDepots);
                $stmt = $db->prepare($baseSql . " WHERE v.vehicle_id = ? LIMIT 1");
                $stmt->execute([$id]);
                $row = $stmt->fetch();
